...
use crate::code::{X, write_name, write_typ, write_escaped};


fn write_pre_de(f: &mut F, name: &str, len: usize) -> Result<()> {
    f.write_all(b"\nimpl _tl::de::Deserialize for ")?;
    f.write_all(name.as_bytes())?;
    f.write_all(b" {\n    const MINIMUM_SERIALIZED_LEN: usize = ")?;
    write!(f, "{len}")?;
    f.write_all(b";\n\n    unsafe fn deserialize(buf: &mut _tl::de::Buf) -> Result<Self, _tl::de::Error> {\n        ")
}
...
fn write_enum_de(f: &mut F, cfg: &Cfg, data: &Data, x: &Enum) -> Result<()> {
    ...
    for variant in &x.variants {
        ...
        write_typ(f, cfg, data, &[], &Typ::Type { index: *variant, params: Vec::new() }, true)?;
        f.write_all(if x.recursive { b"::deserialize_checked(buf)?))),\n" } else { b"::deserialize_checked(buf)?)),\n" })?;
    }

    f.write_all(b"            id => Err(_tl::de::Error::UnexpectedConstructor { id }),\n        }")
}
